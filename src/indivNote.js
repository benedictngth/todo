//fix the notes order + drag and drop will cause notes to disappear and appear randomly
import "./App.css";
import "./index.css";
...
function IndivNote({ index, column, note, setNotes, setDragOrder }) {
    ...
    setNotes((prevState) => {
      let newNote = prevState.map((note) =>
        selectedNote.id === note.id
          ? { ...note, name: inputName, description: inputDescription }
          : note
      );
      return newNote;
    });
  ...
  function deleteNote(selectColumn, selectedNote) {
    ...
    setDragOrder((prevDragOrder) => {
      selectColumn.noteOrder = selectColumn.noteOrder.filter(
        (noteID) => noteID != selectedNote.id
      );
      const newDragOrder = {
        column: {
    ...
    <Draggable draggableId={note.id} index={index}>
      {(provided) => (
        ...
        >
          ...
          <button
            className="button edit"
            onClick={() => deleteNote(column, note)}
          >
            Delete
          </button>
        </div>
